import { dataKrakenTakesData, dataKrakenGivesData,dataKrakenTradesData, userDataResponse } from "../interfaces/interfaces";
...
export async function dataKrakenGives({col}:dataKrakenGivesData = {}) {
    ...
    let data ;
    try {
        const response =  await apiToken.get<userDataResponse>(`/dataKrakenGives?col=${col}`)
